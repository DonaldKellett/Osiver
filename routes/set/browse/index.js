...
module.exports = async function (fastify, opts) {
  fastify.get('/', { schema }, async function (request, reply) {
	...
	return reply.status(404).send({ message: 'The requested resource could not be found' })
      let questionSets = await connection.awaitQuery(`SELECT QuestionSets.*, Accounts.prettyName FROM QuestionSets INNER JOIN Accounts ON QuestionSets.teacherId = Accounts.id`)
      await connection.awaitEnd()
      return questionSets.map(questionSet => {
        let { title } = JSON.parse(fs.readFileSync(path.join(config.OSIVER_DATA_BASE, `${questionSet.id}.json`)).toString())
	return {
	  id: questionSet.id,
          title,
          ownerId: questionSet.teacherId,
          owner: questionSet.prettyName,
          created: questionSet.created,
          graded: questionSet.graded,
